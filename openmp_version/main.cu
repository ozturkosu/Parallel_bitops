	...
	int Vector2_size ,size_t * outVector)
{
		 ...
		 size_t* word_lengths2 = new size_t[Vector2_size]; //vector of length of the words
		 size_t* prefix_sum2 = new size_t[Vector2_size];   // prefix-sum generated form word_lengths1
		 size_t pre_sum2 = 0;
...
int main(int argc, char** argv)
{
    ...
    //pthread_exit(NULL);
		...
		for(int i = 0 ; i<vec2_size;i++)
	  {
		...
		double elapsed_sec = double(t_KERNEL2 - t_KERNEL1) / CLOCKS_PER_SEC ;

		cout<<"Kernel Time :"<< elapsed_sec <<endl;
